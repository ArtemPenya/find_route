...
class AllTestsCase(TestCase):
    ...
    def test_model_train_time(self):
        train = Train(name='t1', from_city=self.city_A, to_city=self.city_B, travel_time=9)
        with self.assertRaises(ValidationError):
            train.full_clean()
        try:
            train.full_clean()
            ...
            self.assertEqual(
                {'__all__': ['Измените время в пути'], 'name': ['Поезд с таким Номер поезда уже существует.']},
                e.message_dict)
    ...
    def test_cbv_detail_views(self):
        response = self.client.get(reverse('detail', kwargs={'pk': self.city_A.id}))
        self.assertEqual(200, response.status_code)
    ...
    def test_valid_route_form(self):
        data = {'from_city': self.city_A.id, 'to_city': self.city_B.id, 'travelling_time': 9,
    ...
    def test_message_error_from_cities(self):
        data = {'from_city': self.city_B.id, 'to_city': self.city_E.id, 'travelling_time': 349,
                'cities': [self.city_C.id]}
        response = self.client.post('/find_routes/', data)
